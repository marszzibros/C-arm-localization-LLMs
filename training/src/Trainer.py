...
class Utils:
    ...
    def format_real_data(self, sample):
        
        with open("../prompts/real_prompt.txt", "r") as f:
            system_prompt = f.read()
    ...
    def sft_real_datasets(self, folder_path="../../data"):
        # Load your CSV into a pandas DataFrame
        df = pd.read_csv(os.path.join(folder_path, "real.csv"), index_col=0)
        df = df[df['mode'] == 'train']
        ...
        def process_batch(examples):
            images = []
            answers = []
            for path, ans_str in zip(examples["SOPInstanceUID"], examples["Target"]):
        ...
        dataset = dataset.map(
            process_batch,
            batched=True,
            num_proc=16,  # adjust depending on CPU availability
            desc="Processing images and labels"
        )
        datasets = []
        for i in range(len(dataset)):
            formatted = self.format_real_data(dataset[i])
...
class Model:
    def __init__(self, cfg, real_eval=False):
        ...
        # dataset
        if real_eval:
            self.dataset = self.utils.sft_real_datasets()
        else:
        ...
        self.model = FastVisionModel.get_peft_model(
            ...
            lora_alpha = lora_config_dict['lora_alpha'],  # Recommended alpha == r at least
    ...
    def SFT(self):
        ...
        self.model.save_pretrained_merged(self.cfg.train.output_dir+str(16)+real, self.processor, save_method = "merged_16bit",)
